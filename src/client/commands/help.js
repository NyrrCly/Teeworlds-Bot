...
export default class HelpCommands {
    ...
    static async sendUtilsCommands(client, author) {
        return Chat.sendMessage(client, `/w ${author} Commands: profile: "profile Aoe", spam: "spam 'SW1PTYK sigma boy'"`);
    }

    static async sendModerationCommands(client, author) {
        return Chat.sendMessage(client, `/w ${author} Commands: ban: "ban 'id time(minutes) reason'", kick: "kick 'id reason'", mute: "mute id time(seconds) reason'"`);
    }
}