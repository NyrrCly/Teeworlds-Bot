...
import { ban, help, kick, login, moderationHelp, mute, profile, utilsHelp } from "./clientCommands.js";
import { coinFlip, eightBall } from "../shared/utils/miniGames.js";

export async function checkCommands(client: TWClient, commandName: string, commandArg: string, message: iMessage) {
    const author = message.author?.ClientInfo?.name ?? "Unknown";
    switch(commandName) {
        case "login": {
            if (isPlayerInLoggedClients(client.client, message.author?.ClientInfo?.name ?? "smallnicknamethatuseintest")) return;
            await login(client, commandArg, message.author?.ClientInfo?.name!);
        }
        case "help": {
            if (commandArg.toLowerCase() === "utils") return utilsHelp(client, author);
            if (commandArg.toLowerCase() === "moderation") return moderationHelp(client, author);
