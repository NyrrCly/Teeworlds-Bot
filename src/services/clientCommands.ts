...
export function help(client: TWClient, author: string) {
  return client.sendMessage(
    `/w ${author} Prefix: ${config.chat.prefix} | categories: Utils, Moderation | Example: help utils`,
  );
}

export function utilsHelp(client: TWClient, author: string) {
  return client.sendMessage(
    `/w ${author} Commands: profile - Example: !profile Aoe | coinflip | 8ball`,
  );
}

export function moderationHelp(client: TWClient, author: string) {
  return client.sendMessage(
    `/w ${author} Commands: ban '<id> <time(minutes)> <reason>' | kick '<id> <reason>' | mute '<id> <time(seconds)> <reason>'`,
  );
}
...
export async function profile(client: TWClient, playerNickname: string) {
  ...
  if (!playerData)
    return client.sendMessage(`Unknown player name: ${playerNickname}`);

  const totalSecondsPlayed = playerData?.general_activity?.total_seconds_played;
  const totalHoursPlayed = totalSecondsPlayed
    ? playerHours(totalSecondsPlayed)
    : "Unknown";

  return client.sendMessage(
    `${playerData.profile.name} profile: points: ${playerData.profile.points} | hours: ${totalHoursPlayed}`,
  );
}
